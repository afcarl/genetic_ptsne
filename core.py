...
# Modification of parametric tsne to have loss logging using built in keras
# callbacks
...
class Parametric_tSNE(object):
    ...
    def __init__(self, num_inputs, num_outputs, perplexities,
        ...
        if all_layers is None:
            ...
            # build our layers
            all_layer_sizes = [num_inputs] + all_layers + [num_outputs]
            # first layer
            all_layers = [layers.Dense(all_layer_sizes[1] , input_shape=(num_inputs,), activation='sigmoid', kernel_initializer='glorot_uniform')]
    ...
    def fit(self, training_data, training_betas=None, epochs=10, verbose=0):
        ...
        # modification for loss history recording
        history = LossHistory()
        self.model.fit_generator(train_generator, batches_per_epoch, epochs, verbose=verbose, callbacks=[history])

        if verbose:
            print('{time}: Finished training on {epochs} epochs'.format(time=datetime.datetime.now(), epochs=epochs))

        # return a history of the loss function's value
